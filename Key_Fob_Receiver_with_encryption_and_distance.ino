...
void broadcast_rx_configuration();
...
//  uint8_t buffer[64];
    ...
    AES_CBC_decrypt_buffer(&ctx, in, message.length());//64);
...
void receive_data()
{
  if (received) {
    ...
    memset(in,'\0',sizeof(in));//message.length());
...
{
      //Serial.print("from: ");
      uint receiver_tag_id = 0;
      receiver_tag_id = DW1000Ranging.getDistantDevice()->getShortAddress();
  //Serial.print(DW1000Ranging.getDistantDevice()->getShortAddress(), HEX);
  memset(arr,'\0',sizeof(arr));
  sprintf(arr,"%X",receiver_tag_id);
  //Serial.print(arr);
  //Serial.print(", ");
